...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function loadInsights() {
      if (!user || chartData.every(d => d.earnings === 0)) return;
      try {
        const result = await getBusinessPerformanceInsights({
          ...
          hourlySales: [ { hour: 12, salesCount: 5 }, { hour: 18, salesCount: 15 } ],
        });
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-4">
                {isAiLoading && totalThisWeek > 0 ? (
                  ...
                  <p className="text-[12px] font-bold text-emerald-900/60 text-center py-4">
                    Insights will appear once you have sales data.
                  </p>
                )}
              </div>
            </CardContent>
